...
import { generatedData } from "~/_devmode/generator/generator";
...
export default function useAuth () {
	...
	const signIn = async () => {
		try {
			...
			await initLocalDb();
			await saveDocumentsToLocalDb("profile", [randomUserProfile]);
			await syncLocalDB();
	...
	const restoreSession = async () => {
		if(appState.value.authenticated) {
			let profile = await getDocumentsFromLocalDb("profile")
